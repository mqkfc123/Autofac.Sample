...
namespace Dragon.Infrastructure.Utility.Cache
{
    ...
    public class CookieHelper
    {
        ...
        /// 获取所有 Cookie
        /// </summary>
        public static Dictionary<string, string> GetAllCookie()
        {
            var cookies = new Dictionary<string, string>();

            foreach (var key in HttpContext.Current.Request.Cookies.AllKeys)
            {
                var item = HttpContext.Current.Request.Cookies[key];
                if (item != null)
                    cookies.Add(key, item.Value);
            }
            return cookies;

...
SWFUpload.prototype.initSettings = function (userSettings) {
    ...
    // Flash Settings
    //this.ensureDefault("flash_url", (window.ApplicationPath || "/") + "Themes/TheAdmin/lib/swfupload/flash/swfupload.swf");
    //this.ensureDefault("flash9_url", (window.ApplicationPath || "/") + "Themes/TheAdmin/lib/swfupload/flash/swfupload_fp9.swf");
    this.ensureDefault("flash_url", (window.ApplicationPath || "/") + "Scrpits/widget/swfupload/flash/swfupload.swf");
    this.ensureDefault("flash9_url", (window.ApplicationPath || "/") + "Scrpits/widget/swfupload/flash/swfupload_fp9.swf");
...
function preLoad() {
    if (!this.support.loading) {
        alert("你需要安装Flash播放器9.028版本或以上版本。");
        return false;
    }
}

function loadFailed() {
    alert("加载 SWFUpload 上传按钮时出了错");
}
    ...
    try {
        ...
        switch (errorCode) {
            case SWFUpload.QUEUE_ERROR.QUEUE_LIMIT_EXCEEDED:
                errorMessage = "错误提示：一次性最多上传" + message + "个文件";
                break;
            case SWFUpload.QUEUE_ERROR.FILE_EXCEEDS_SIZE_LIMIT:
                errorMessage = "错误提示：文件" + file.name + "大小超出"; // +message;
                break;
            case SWFUpload.QUEUE_ERROR.INVALID_FILETYPE:
                errorMessage = "错误提示：文件格式不正确!";
                break;
            default:
                errorMessage = "错误提示：未知错误，错误代码" + errorCode; // +"," + message;
                break;
        }
        alert(errorMessage);
...
function fileDialogComplete(numFilesQueued) {
    try {
        if (numFilesQueued > 0) {
            //  $(this.customSettings.upload_target).html('<div style=" width: 150px; height: 2px; border: solid 1px #fff; background-color: #fff;"><div style="height: 2px; background-color: Orange; width:0"></div></div>');
            this.startUpload();
...
//图片上传
function fileDialogCompleteImg(numFilesQueued) {
    /// <summary>当选择文件对话框关闭，并且所有选择文件已经处理完成（加入上传队列成功或者失败）时，触发该事件</summary>
    /// <param name="numFilesQueued" type="Int">此次选择的文件中成功加入队列的文件数目</param>

    try {
        if (numFilesQueued > 0) {
            $(this.customSettings.upload_img).attr("src", "/Admin/Images/commom/loading.gif");
            this.startUpload();
            //this.setButtonDisabled(true);
        }
    } catch (ex) {
...
function uploadProgress(file, bytesLoaded, total) {
    try {
        ...
        // $(this.customSettings.upload_target).find("div div").css("width", percent + "%");
    } catch (ex) {
        this.debug(ex);
    }
}
...
function uploadError(file, errorCode, message) {
    /// <summary>无论什么时候，只要上传被终止或者没有成功完成，那么该事件都将被触发</summary>
    /// <param name="file" type="Object">上传文件对象</param>
    /// <param name="errorCode" type="Int">错误代码</param>
    ...
    try {
        ...
        $(this.customSettings.upload_img).hide();
    } catch (ex3) {
        this.debug(ex3);
    }
}
...
function uploadComplete(file) {
    /// <summary>当上传队列中的一个文件完成了一个上传周期，无论是成功(uoloadSuccess触发)还是失败(uploadError触发)，此事件都会被触发，这也标志着一个文件的上传完成，可以进行下一个文件的上传了。</summary>
    /// <param name="file" type="Object">上传文件对象</param>
...
function fadeIn(element, opacity) {
    /// <summary>淡出显示</summary>
    /// <param name="element" type="Object">Html元素对象</param>
    /// <param name="opacity" type="Int">透明度</param>

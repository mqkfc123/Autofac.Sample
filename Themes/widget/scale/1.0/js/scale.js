	...
	ImagesZoom.prototype = {
		...
		init: function(param){
			...
			var imgList   = document.querySelectorAll(params.elem + " img"),
				...
				zoomPrev   = document.querySelector(".imgzoom-pack .imgzoom-prev"),
				zoomNext = document.querySelector(".imgzoom-pack .imgzoom-next"),
                aoomTurn = document.querySelector(".imgzoom-pack .imgzoom-turn"),
				imgSrc    = "";
			...
			zoomPrev.addEventListener("click", function(){
				var isli = imgIndex.parent().is('li');
				if(isli){
					if(imgIndex.parent().prev().is('li')){
						imgIndex = imgIndex.parent().prev().find('img');
						zoomImg.src = imgIndex.attr('src');
					}else if(imgIndex.parents('.img-scale').prevAll('.img-scale:first').is(':visible')){
						if(imgIndex.parents('.img-scale').prevAll('.img-scale:first').find('li:first').hasClass('file-btn')){
							return;
						}else if(imgIndex.parents('.img-scale').prevAll('.img-scale:first').find('li:last').hasClass('file-btn')){
							imgIndex = imgIndex.parents('.img-scale').prevAll('.img-scale:first').find('li:last').prev().find('img');
						}else{
							imgIndex = imgIndex.parents('.img-scale').prevAll('.img-scale:first').find('li:last').find('img');	
						}
						zoomImg.src = imgIndex.attr('src');	
					}
				}else{
					var objectindex
					$(".img-scale").each(function(index, value){
						if (imgIndex.parents('.img-scale').is(value)) {
							objectindex = index;
						}
					});
					if(imgIndex.prev().attr('src')){
						imgIndex = imgIndex.prev();
						zoomImg.src = imgIndex.attr('src');
					}else if(objectindex - 1 >= 0){
						imgIndex = $(".img-scale").eq(objectindex - 1).find('img:last');
						zoomImg.src = imgIndex.attr('src');
					}
				}
				document.removeEventListener("touchmove", self.eventStop, false);
			...
			zoomNext.addEventListener("click", function(){
				var isli = imgIndex.parent().is('li');
				if(isli){
					if(imgIndex.parent().next().is('li') && !imgIndex.parent().next().hasClass('file-btn')){
						imgIndex = imgIndex.parent().next().find('img');
						zoomImg.src = imgIndex.attr('src');
					}else if(imgIndex.parents('.img-scale').nextAll('.img-scale:first').is(':visible')){
						if(imgIndex.parents('.img-scale').nextAll('.img-scale:first').find('li:first').hasClass('file-btn')){
							return;
						}else{
							imgIndex = imgIndex.parents('.img-scale').nextAll('.img-scale:first').find('li:first').find('img');	
						}
						zoomImg.src = imgIndex.attr('src');	
					}
				}else{
					...
					if(imgIndex.next().attr('src')){
						imgIndex = imgIndex.next();
						zoomImg.src = imgIndex.attr('src');
					}else if(objectindex + 1 < $(".img-scale").length){
						imgIndex = $(".img-scale").eq(objectindex + 1).find('img:first');
						zoomImg.src = imgIndex.attr('src');
					}
				}
				document.removeEventListener("touchmove", self.eventStop, false);
			...
			var current = 0;
			if (aoomTurn) {
			    aoomTurn.addEventListener("click", function () {
			        var imgLi = $(".imgzoom-img").find("img");
			        var img = $(imgLi).get(0);
